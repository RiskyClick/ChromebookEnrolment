...
#define device_version 78 // Change to the ChromeOS version you expect to use with Centipede; Changes have been reported in the following ranges 58-68, 69, 70
...
#define selected_on_boot 0 //[0,1,2,3]  Active location when device poweron : 3 - "Let's go"(Default), 2 - accessibility, 1 - language, 0 - menu 
...
#define VERSION_87 (device_version >= 87)
...
void setup()
{
  ...
  wait(15); // Wait for all services to finish loading
}
...
void loop() { // Main Function - workflow is called within loop();
  if (digitalRead(buttonPin) == 1 ) { // Check for debugging. If not debugging, run the program
    ...
    wait(20); // wait for Enrollment to complete
    Keyboard.write(KEY_ENTER);
